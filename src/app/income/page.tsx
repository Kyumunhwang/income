...
export default async function IncomePage({ searchParams }: Props) {
  ...
  if (q) {
    incomeData = incomeData.filter((row: any) => 
  ...
  return (
    <div className="max-w-[1280px] mx-auto">
      <div className="mb-10 flex justify-between items-end">
        <div>
          <h2 className="text-[32px] font-bold font-sans text-on-surface mb-2 tracking-[-0.02em]">Income Management</h2>
          <p className="text-[16px] font-sans text-outline">Track your income sources</p>
        </div>
        <NewIncomeButton />
      </div>
      ...
      {incomeData.length === 0 ? (
        <div className="bg-surface rounded-xl p-16 flex flex-col items-center justify-center border border-outline-variant/30 text-center shadow-sm">
          <div className="w-20 h-20 bg-[#f0edef] rounded-full flex items-center justify-center mb-6">
            <span className="material-symbols-outlined text-[40px] text-outline">payments</span>
          </div>
          <h3 className="text-[20px] font-bold text-on-surface mb-2">No income records found</h3>
          <p className="text-[16px] text-outline max-w-md mb-8">
            You haven't recorded any income yet. Add your first income to start tracking your cash flow.
          </p>
          <NewIncomeButton isFirst={true} />
        </div>
      ) : (
        <div className="bg-surface rounded-xl shadow-sm overflow-hidden border border-outline-variant/30">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {incomeData.map((row: any, index: number) => (
                <tr key={index} className="border-b border-outline-variant/30 hover:bg-[#f6f3f5] transition-colors">
                  ...
                  <td className="py-4 px-6 text-[15px] font-bold text-success font-mono text-right">
